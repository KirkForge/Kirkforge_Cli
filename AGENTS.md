...
- **`lessons.md` is TRACKED on dev** (the "gitignored" claim went stale): append-only per session. Parallel worktrees appending to it WILL conflict on merge — resolve by keeping both sides. Fold permanent conventions into `AGENTS.md` at session close regardless; `lessons.md` is per-session scratch history.
- **Interrupted sessions leave three-part residue** (learned closing out the rate-limit-killed WO 43 session): uncommitted worktree diffs, WO files flipped Done without README index rows (drift test red), and state.md "Pending" contradicting its own commit messages. Check all three against `git log`/code before trusting any of them. The Windows cross gate is the check most often skipped when a session dies mid-flight — run it FIRST on resume.
...
- **Refactor-on-touch policy for the top-5 long functions** (context only — no standalone refactor WO; split when you're already editing for another reason, not as a drive-by): `handle_input_key` (`tui/keys/mod.rs`, ~746 lines), `Executor::run` (`session/executor/loop_.rs`, ~434), `dispatch_turn_event` (`tui/events.rs`, ~390), `dispatch_slash_command` (`tui/keys/slash_commands.rs`, ~378), `init_default_verifiers` (`session/executor/mod.rs`, ~351). When a change lands in one of these, extract the touched region into a named helper before the function grows further. Do NOT launch a big-bang refactor of any of these — the policy is opportunistic extraction, not a dedicated workorder.
...
7. **BRANCH FLOW (NON-NEGOTIABLE)**: `push to dev` → `wait for CI green (gh run watch --exit-status)` → `only then push main`. NEVER push main and dev simultaneously. NEVER push main before dev CI is green. If dev CI is red, FIX IT before pushing main. This rule has been violated enough times that it is now a REGRESSION to break it. The sequence is: (1) `git push origin main:dev`, (2) `gh run watch <run-id> --exit-status`, (3) if exit 0: `git push origin main`, (4) if exit non-zero: fix the failure, re-push dev, re-watch. There is no shortcut. There is no "I'll push both and check later." There is no "the dev push will probably be green." You watch. You confirm. Then you push main.
