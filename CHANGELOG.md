...
### Added
- WO 29.3: New `kf-routing` workspace crate — pure Rust port of the orchestrator's pure modules (~1,750 LOC across 7 files). Ports `routing-engine.ts`, `classifier.ts` + `classifier-nlp.ts`, `correction-loop.ts` + `truth-model.ts`, `task-profile.ts` + `cost.ts`, and `path-safety.ts` from `npm/kf-plugin`. 100 unit tests; identical-behavior FNV-1a vectorizer, TF-IDF NLP classifier, single-precedence truth table, and atomic artifact writes. Foundation for WO 29.7 orchestrator port. No model calls, no event bus.
