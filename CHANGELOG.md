...
### Added
- WO 29.5: Ported `@kirkforge/core-rbac` to a new workspace crate `crates/kf-rbac/`. Covers RBAC (4 roles × 16 permissions, deny-by-default via static slices), timing-safe API-key auth (`subtle::ConstantTimeEq` with right-aligned padding), and OIDC JWT/JWKS verification (`jsonwebtoken` + `reqwest` for discovery). 58/58 TS tests ported (46 RBAC + 12 JWT). `ALLOWED_ALGORITHMS` keeps all 10 policy algorithms; `Validation::algorithms` is derived per key family (RSA→RS*/PS*, EC→ES256/ES384, Ed→EdDSA). DEFERRED: ES512 verifier coverage — `jsonwebtoken` has no ES512 variant (bundles `p256`/`p384` only); ES512 tokens are rejected at header parse, not verified. Tracked in state.md pending. New deps: `jsonwebtoken = "9"`, `subtle = "2"` (dev: `rsa`, `p256`, `rand`, `base64`).
