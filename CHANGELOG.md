...
### Fixed
- WO 30 (TUI bug hunt): interface bugs found via selftest extension + code review. (1) `/quit` (alias of `/exit`) was unreachable from the slash menu and Tab completion — `complete_command` only matched each command's primary trigger, so typing `/q` showed nothing; now matches ALL aliases (`flat_map` over `triggers`). (2) Ctrl+C while generating cancelled silently, so a "did it work?" second press quit the app by accident — the cancel now announces `⛔ Generation cancelled. Press Ctrl+C again to quit.` (mirrors the persona-cancel path). (3) `token_stream_stress` selftest had a stale inverted assertion claiming the long-message tail was clipped, but commit 2f844f6 fixed `max_scroll` to count wrapped rows; restored the positive visibility contract and fixed the now-incorrect header/first-token assertions (they correctly scroll off the top on a tall message under auto_scroll). Selftest extended 12→21 scenarios (slash-menu alias, tab-panel render, tab-switch scroll preservation, /help render, streaming markdown, Enter-empty no-op, Ctrl+C cancel/quit, /exit+/quit end-to-end routing). Gate-required scope creep: 2 pre-existing clippy/fmt violations in `widgets/chat/{mod.rs,lines.rs}` (manual_div_ceil→`div_ceil`, uninlined_format_args) from earlier commits that blocked the gate — pure mechanical fixes, no logic change.
