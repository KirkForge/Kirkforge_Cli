...
### Changed
- WO 33.14: replaced real subprocess/DNS I/O in unit tests with fakes + gates. Injected a `DnsResolver` trait into `WebFetch` (production `SystemResolver`, tests `NxdomainResolver` for instant NXDOMAIN) — un-ignored 5 web_fetch tests that did real DNS NXDOMAIN (~10s each). Gated 4 slow subprocess tests with `#[ignore]` + reasons (`bash_jobs` timeout-reap, `hooks` slow-hook, `bash_runner` kill-descendants, `jobs::runner` bash-job-timeout). Kept 1 real DNS smoke test (`localhost`). No production behavior changed.
...
- Removed redundant `cargo check` step from the `clippy` job in `ci-pr.yml` and `ci-merge.yml` (WO 33.4). Clippy with `-D warnings` already fails on any compile error; the same-scope `cargo check --workspace --lib --bins --features e2e-tests --locked` step re-ran the build without adding coverage. Windows release build confirmed split: `ci-merge.yml` `windows` job runs tests only, `ci-nightly.yml` `release-build` matrix covers Windows (WO 33.9, verified already in place from WO 33.3).
