...
### Fixed
- WO 38.13: docs truth pass + WO status reconciliation. (1) TECHNICAL.md overclaims fixed: deleted stale personas-Anthropic-only paragraph (WO 26.6 closed the gap); removed `kf-budget` binary claim (no standalone binary exists); corrected shell-plugin fallback claims (shell trees deleted in WO 29.9); bash allowlist "deferred" → shipped (WO 32.18); test count updated (~3,600 src + ~960 crates); hooks event list deduplicated; stale bench-workflow claim corrected. (2) ADR-019/026/040 amendment notes added (VSCode extension archived, not status flips). (3) WO status reconciled: 104 file headers synced with README index (63 files updated to match README, 41 README rows updated to match shipped files); 27.2/29.0/32.0 overview headers closed; 33.11 deduped (stale duplicate archived). (4) Silent deferrals surfaced: native.rs `tools` command rewritten to Rust runtime truth (7 dead TS lint engines removed); `doctor` C/C++/Go/Rust/SQL scanning claims corrected (only JS/TS/Py); WO 32.6 status → "Partially done — framework only; live run tracked in WO 39.1"; stale Cargo.toml comment fixed; jobs/mod.rs stub doc corrected; verifier/bus.rs register doc updated. (5) config.toml.example drift fixed: defaults 200/20/120 (was 100/5/30); minify_write_side now true; enabled_plugins example commented (silent disable of stratum+budget); [session] worktree_enabled knob added. (6) New WO drift test in `kf-budget-core/tests/adr_xref_drift.rs` — extends the ADR drift pattern to WO status (file header ↔ README index must agree on leading keyword).
