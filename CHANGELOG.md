...
### Added
<<<<<<< HEAD
- WO 28.3: Split the `turn.rs` god-object. `dispatch_tool_call_batch` is now a 28-line orchestrator over three phase methods (`prepare_batch`/`spawn_batch`/`collect_batch`) moved to `executor/dispatch.rs`. `pre_run_verdict` + `PreRunVerdict` moved to a new `executor/pre_run.rs`. `stream_iteration`'s 170-line memory-context/config-snapshot preamble moved to `build_stream_preamble` in a new `executor/stream.rs`; the SSE-driver loop stays in `turn.rs`. Pure move, no behavior change. `turn.rs` drops 2087→1195 LOC.
||||||| 5a6c32d
=======
- WO 28.10: Gated the `tests/e2e/` integration suite behind a new `e2e-tests` cargo feature (`required-features` on the `[[test]] e2e` target). `cargo test --workspace` and `cargo check --all-targets` skip the e2e crate by default; CI runs it in a dedicated `e2e` job with `--features e2e-tests`. The `quality` job's Clippy + typecheck pass the same feature so the gated crate stays under the lint gate.
- WO 28.11: Extended `scripts/check-artifact-consistency.sh` with check #9 (count of `| `<slug>.toml` |` rows in `docs/TECHNICAL.md` vs `benches/tasks/*.toml` directory) and extended the CI `fmt` job's TOML-parse Python step to assert required keys (`name`, `difficulty`, `prompt`) on every bench task. R3 (name-set manifest) deferred — see WO 28.11 status.
- WO 28.12: Extended `scripts/check-artifact-consistency.sh` with check #10 (identifier-position grep of `src/` + `crates/` for `use`/`mod`/`extern crate` of retired identifiers `plugin3`, `kfd`, `kf_code_video`, `kf_code_draw`, `kf_code_plugin`; also `crates/*/Cargo.toml` dep entries). Closes the dead-ref firewall gap at the source boundary. `stratum` intentionally excluded (live feature).
>>>>>>> wo28e
