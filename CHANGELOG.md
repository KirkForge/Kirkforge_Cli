...
- WO 39.1: bench suite repair + first cross-tool run. (1) Fixed `benches/tasks/use_workflow_run.toml` using nonexistent `output_contains` verify variant → suite now loads all 30 tasks. (2) Fixed 5 free-win verifies (`add_doc_comment`, `fix_failing_test`, `fix_clippy_warning`, `add_adr`, `add_enum_variant`) that passed on the UNEDITED setup — each verify now checks something the task actually changes. (3) Tagged 4 kf-only tasks (`use_budget_check`, `use_lsp_query`, `use_stratum_compress`, `use_workflow_run`) with `kf_only = true` so the cross-tool subset excludes them. (4) Extended CI Python lint to validate `[verify].type` against the known `VerifySpec` variants, catching typos before CI runs. (5) `bench run` exit code is non-zero on 0% success (already shipped via WO 38.10, verified). (6) Added `bench export-tasks <dir>` — materializes setup files + `PROMPT.txt` into a workspace dir so external agents can run against them. (7) Replaced the broken `scripts/run-cross-tool-bench.sh` kf-code half with `kf-code bench run --output`; fixed nonexistent `--prompt` flag, wrong tasks dir, exit-code-as-success. Phase 3 (generic external runner for `claude -p` / `codex exec` / `opencode run`) and Phase 4 (same-model LiteLLM gateway) deferred — tracked in WO 39.1.
- WO 38.7: resource lifecycle + worktree sweep. (1) `WorktreeSession::drop` now falls back to `fs::remove_dir_all` + `git worktree prune` when `git worktree remove` fails (repo moved/unavailable), so the worktree dir and `.git/worktrees` registration no longer leak. (2) Boot sweep `sweep_stale_worktrees()` removes `kf-code-session-*` dirs in `$TMPDIR` older than 24h (age-guarded like the undo orphan-snap sweep), called at session start before `WorktreeSession::create`. (3) MCP client `connect()` kills the orphaned child via `kill_process_group` when `stdin.take()`/`stdout.take()` fails instead of `?`-returning and leaking the process. (4) Jobs daemon writes its PID file after `UnixListener::bind` (mirrors `src/daemon/server.rs:63-71`) so a failed bind leaves no stale PID. (5) `conversation.rs` and `jobs/store.rs` (save + record_run) remove the `.tmp` file on write/rename failure (mirrors `atomic_write.rs:46-49`) instead of leaving it behind. Task 5 (verifier/formatter subprocess bounds) already landed via WO 38.3 on this branch — verified and skipped.

### Added
- WO 39.3: Claude compat phase 2 — dynamic agents + tool-name aliases. `.claude/agents/*.md` files load into an `AgentRegistry` (`src/session/agents.rs`); an unknown `task` persona matching an agent name gets the agent's system prompt as preamble and its `tools` frontmatter (translated through the `CLAUDE_TOOL_ALIASES` table: Read→read_file, Bash→bash, Task→task, …) as the toolset allowlist. The agent's `model` frontmatter overrides the per-call model. A prompt-alias suffix is appended so the model's prose references to Claude tool names map to native names. Trust gate: the workspace `.claude/agents/` dir (model-writable in-session) is refused unless `plugin_trust_workspace = true` or the dir is under the data directory — same threat model as workspace plugins. The `task` tool description lists discovered agents.

### Added
- WO 38.6: crash-corruption recovery. (1) Torn conversation log lines now heal on `open`: when `load_messages` reports corrupt==true and at least one valid message survived, the log is atomically rewritten from the valid messages so the torn tail is gone before the next `append` (O_APPEND, no leading newline) can fuse onto it — previously both the crashed message and the next one were permanently unparseable. (2) `StartedEmpty` (corrupt log, no usable checkpoint) is now surfaced: `run_session` prints an eprintln naming the path and stating the corrupt original was left in place, and the TUI mirrors it with an in-app banner (the pre-TUI eprintln is lost to the alternate screen); line-mode is covered by the run_session eprintln. (3) `save_config` is now atomic (temp + fsync + `rename_with_retry`): a torn write leaves the original config intact instead of a truncated file that loads as all defaults and permanently erases the user's config on the next save.
- WO 38.8: wire the budget guard into production — the thesis runs for users now. `set_budget_stores` / `set_stratum_store` had zero production callers; `run_session.rs` built `SessionStores` and dropped it; `apply_budget_slice` was dead code on the `None/None` path. Fixed: `run_session.rs` passes `SessionStores` to `run_tui` / `run_line_mode`, which call `executor.attach_session_stores(stores)` after `set_session_id`. `attach_session_stores` runs `init_from_config`, registers the budget hooks (session-start, post-tool-bash, post-tool-write_file, pre-compact), and registers the Stratum slice-compression listener keyed by session_id. The sliced-listener registry (`SLICED_LISTENERS`) is now keyed by session_id (`HashMap<String, Vec<Listener>>`) instead of an append-only global `Vec` — the old `ceiling:` comment's premise was inverted (oldest listener won, not dead code), so concurrent executors (personas, subagents, `/plugins` reload) dispatched slices to session #1's listener. `clear_session_sliced_listeners(session_id)` on executor `Drop` prevents cross-session leaks. `reload_plugins` clears + re-registers the session's listeners. New tests: `budget_wiring_test.rs` — slicing fires through the production `attach_session_stores` path (not the manual `set_budget_stores` of 35.5); multi-executor isolation (two executors, slices land in their own stores). 35.5's `context_economics_test` updated to the session-keyed API and still green.
- WO 40.2: Windows cross-compile gate in `scripts/ci-local.sh`. A mandatory `cargo clippy --target x86_64-pc-windows-gnu --workspace --all-targets -- -D warnings` step now runs after the Linux clippy step, catching every cfg(unix) gap (unix-only imports/consts/test helpers used ungated) before push. This ends the recurring 25+ `fix(windows)` commit pattern from Aug 2026 — Linux clippy was blind to those gaps. AGENTS.md §4 now requires running `scripts/ci-local.sh` before every commit. The current dev tree passes both Windows clippy and `cargo check --target x86_64-pc-windows-gnu` clean (the prior WO 38.1/38.3 commits already gated all unix-only items); the gate is the structural defense against regression.
- WO 40.3: nextest profiles + timeout policy. `.config/nextest.toml` now defines `ci-fast` (30s timeout, terminate-after=1, grace 10s, fail-fast, `not test(#ignore)`), `ci-full` (60s, grace 15s, no-fail-fast, skip `#[ignore]`), `integration` (120s), `e2e` (300s), and `nightly` (600s, no `#[ignore]` filter — runs everything via `--run-ignored all`). All timeouts use `slow-timeout = { period, terminate-after = 1, grace-period }` so tests are killed at the boundary, not just flagged SLOW. CI workflows reference profiles by name (no inline `--config`); `scripts/test-fast.sh` → `--profile ci-fast`, `scripts/test-full.sh` → `--profile ci-full`, `scripts/ci-local.sh` uses profiles with `cargo test` fallback. ci-merge `full-tests` job adds a slow-test report (top 20) posted to `$GITHUB_STEP_SUMMARY`; ci-pr `fast-tests` job warns on any test >10s. Nightly `ollama` job switched from `cargo test --include-ignored` to `cargo nextest run --profile nightly --run-ignored all`.

### Added
- WO 39.2: Claude compat phase 1 — skills, commands, .mcp.json discovery. (1) `parse_frontmatter` derives `/<name>` when `trigger` is empty, so every stock `.claude/skills/*/SKILL.md` (including this repo's 6 GitNexus skills) becomes an invocable slash command — previously skills without an explicit trigger registered unreachable (dispatch is `get_by_trigger` only). (2) `.claude/commands/**/*.md` (+ optional `~/.claude/commands`) files register as skills: filename stem → trigger, `$ARGUMENTS`/`$1..$9` → `{{args}}` placeholder rewrite. Optional YAML frontmatter parsed for name/description/model; absent frontmatter falls back to the stem. (3) `.mcp.json` in the project root is parsed (`mcpServers` → `Vec<McpServerConfig>`, command/args/env → stdio, url/token → http) and merged before `McpClientManager::new`. Security gate: `tools.load_project_mcp_json` config flag (default on) + first-load-per-project approval prompt persisted in the data dir (`approved_mcp_projects.json`) — a cloned repo's `.mcp.json` is attacker-controllable spawn config. `resolve_project_mcp` is a pure function over `(flag, doc, approved)` so the gate is testable without a terminal. DEFERRED: the TUI approval modal (phase 1 prompt-then-record via stderr; a real yes/no modal lands when the TUI gains a generic prompt component — tracked in WO 39.x). New tests: 7 skills tests (trigger derivation, real Claude SKILL.md fixture, command loader, placeholder rewrite, commands dir scan), 12 mcp_project tests (stdio+http parse, env passthrough, type override, missing/empty/malformed, resolve blocks/admits/config-off/no-file, approval persistence round-trip + reload). `CONFIG_FIELD_COUNT` 104 → 105, `MERGE_TOML_EXPECTED` 97 → 98, `ENV_OVERRIDE_EXPECTED` 93 → 94.
- WO 40.5: global state isolation for parallel-test correctness. CwdGuard (RAII, shared serializing tokio mutex) hoisted to `test_util.rs` from a private copy in `workflow.rs`; 6 CWD-mutating tests converted (workflow×2, glob×1, tui/commands/workflow×3). `run_shell_child_env_scrubs_secret_vars` converted from raw `set_var`/`remove_var` to `EnvGuard`. `DataDirGuard` (thread-local override) added to `session::data_dir()` so tests inject a temp data dir without mutating `KF_CODE_DATA_DIR` env — 19 tests converted (session_index×15, session/mod×4), removing them from the `test_data_dir_lock` contention pool. `#[serial]` (serial_test dev-dep) on 5 tests that mutate the `SESSION_MODE` OnceLock registry (session_mode, stratum×2, budget×2) — injection impossible because production code reads it cross-module. Grep for unguarded `set_var`/`remove_var`/`set_current_dir` in test code: zero results.
- WO 38.5: adapter robustness — 10 items from the adapter audit. (1) A turn-fatal adapter/config error (429/5xx past retries, 401/403/404, missing key) now emits `Error` + `TurnComplete` and keeps the session alive for retry instead of killing the executor loop — exit is reserved for channel closure. (2) Esc epoch window: the stale Esc queued before a fresh input is drained in the input arm before the new cancel token installs, so it can't abort the next turn (WO 38.4 #3 drain-before-install variant). (3) Anthropic-family usage capture: the SSE parser accumulates usage across `message_start` (input side) + `message_delta` (final `output_tokens`); `message_stop`'s own value wins only if a non-conforming server sends one. `TokenUsage` gained `cache_write_tokens` (`cache_creation_input_tokens`), billed additively at the write rate. CostStats now fires on Anthropic/Bedrock/Vertex instead of $0. Wire-shape test fixtures rewritten to the real API frames (the old tests pinned usage on `message_stop`, which the real API never sends). (4) Pricing table: `claude-*` rows for real ids (`claude-sonnet-4`, `claude-opus-4`, `claude-3-5-*`, `claude-3-*`); Bedrock `anthropic.claude-*` namespace stripped before matching; longest-prefix wins; legacy short prefixes kept. Config-driven `[price_overrides."<prefix>"]` override the table (longest prefix); unmapped models hit the $0 sentinel with a once-per-model WARN instead of silent $0. (5) Truncation semantics unified: a channel close without a terminal event mirrors the cancel path (persist partial assistant message + placeholder tool results, `Done{Error}`) instead of laundering a half-reply into `Finished(Stop)`. The Anthropic and OpenAI-compat parsers both stop synthesizing `Done(Stop)` after a transport error. (6) Cache poisoning: the response cache never stores a stream containing any `Error` event or ending in `Done{Error}`; replay zeroes the `Done` usage so a cache hit does not re-bill the original turn's tokens. (7) Zero-arg tool calls: Anthropic `content_block_stop` flushes a pending `tool_use` unconditionally (missing input → `json!({})`); OpenAI-compat empty-args-with-name → single `json!({})` invocation instead of the "no parseable tool calls" error loop. (8) `openai_api_key` threaded through the constructor and resolved per request (config → `OPENAI_API_KEY` env → no header) so hosted OpenAI-compat endpoints don't 401 into a dead session. (9) Bedrock envelope extraction operates on raw bytes (`Deserializer::from_slice` + `byte_offset()`), so non-UTF8 preludes can't shift offsets and corrupt frame boundaries. (10) OpenAI-compat body sends `stream_options: {include_usage}`; the parser holds `Done` until the post-finish usage frame or `[DONE]` so trailing usage is not dropped.
- WO 38.4: orchestration correctness (adversarial review fixes). (1) `/workflow cancel` on the parallel path is real — the TUI stores the live `Arc<ParallelOrchestrator>` in state and `handle_cancel` calls `cancel_all()` (previously a lying no-op: the shared flag was only read by the sequential runner and `cancel_all` had zero production callers); `cancel_all` also arms a pipeline-level flag that `run_pipeline` checks before each phase, closing the cancel-during-scout window where the not-yet-registered reviewer handle was a blind spot. A prior-phase error short-circuits instead of being stringified into the next phase's handoff — a failed scout no longer burns two more full sessions on a dead provider, and `ParallelResult.aborted` renders `ABORTED` (no lying success UI). (2) Identities mint from the process-global `NEXT_TASK_ID` counter (WO 37.1), not `pid+millis`: two `task` calls in one assistant message no longer collide on the temp dir (shared `conversation.ndjson`, first finisher deleting it under the sibling) or worktree path (stale recovery force-removing a LIVE sibling); a pre-existing temp dir is a hard error, not silent sharing. (3) Handoffs are bounded (8KB char limit) and fenced as untrusted data (`<<<BEGIN UNTRUSTED HANDOFF>>>` delimiters + instruction to ignore embedded directives) — a scout summary is model output shaped by repo contents, so raw injection was trusted-position prompt injection, strictly worse than tool-result injection; `extract_patch` splits at the LAST marker so a model-echoed marker cannot shadow the real appended patch. (4) Cancel is transitive to nested subagents: `TaskMetadata.parent_task_id` records the spawning task and `cascade_parent_cancel` links the parent executor's live token to the nested task's cancel pair (flag + token) — an outer cancel fires the child's pair and the nested `run_task` exits cooperatively (both background and foreground nested tasks). The Esc-then-input window (item 3 in the WO) was reassigned to the WO 38.5 adapter agent and landed there in `src/session/executor/loop_.rs`.
- WO 38.3 (TUI discipline): `/gh`, `/jobs run-now`, `/commit --push`, `/test`, and `!` (both the direct path and the approval-Y path) no longer await subprocess/network I/O on the TUI event-loop task. Each dispatch spawns its work (`tokio::spawn`; `/gh`'s sync `gh` CLI calls run on `spawn_blocking`) and reports the formatted result through a new background-completion channel (`BgCmdDone`, drained by the event loop alongside the persona channel — the `/workflow run` pattern applied to slash commands). `test_in_progress` is set inline at dispatch and cleared when the completion lands; the approval-Y path pushes the same collapse-UX tool entry it did before, just asynchronously. The approval_keys comment claiming the inline await "keeps draining executor events" is corrected — it did not. A stalled `gh api`, a 1-hour job run, `git push`, or a 5-minute `cargo test` now leaves the TUI live (render, Esc, spinner) instead of freezing it.
- WO 38.3 (subprocess bounds): every unbounded subprocess wait in the executor turn path is now bounded and killable. The correction-loop formatter (`apply_command_fix`) gets the hooks treatment — `kill_on_drop`, null stdin, own process group, 5s `tokio::time::timeout` with group kill. The plugin verifier (`kf-plugin-host::PluginVerifier::run`, run under the verifier-bus mutex on every file write) gets a watchdog thread that kills the verifier's process group after 5s and fails closed (`VerifierError::TimedOut` → Error verdict) — a hung verifier script can no longer hold the bus lock. `web_fetch` resolves DNS on `spawn_blocking` (a wedged getaddrinfo no longer blocks a runtime worker), fails CLOSED on resolver errors for non-literal hosts (was fail-open), and `is_internal_addr`'s V6 arm checks `to_ipv4_mapped()` so `http://[::ffff:169.254.169.254]/` is denied. Worktree git ops (`create`, `diff_patch`) run via `spawn_blocking`; the update client and the web_fetch fallback clients set builder timeouts.
- WO 38.3 (completion): fixed three test-compile breakages the prior WIP commit left in the tree (worktree tests missing `use std::process::Command` after the production import was removed; `empty_resolution_passes_guards_unpinned` used `assert_eq!` on `reqwest::Client` which does not impl `PartialEq`; `subagent_lifecycle_test.rs` called the now-async `WorktreeSession::create` without `.await`). Reverted an erroneous `kf-budget-core` README test-count bump (883→882; no `#[test]` was added to that crate by this WO). Flipped WO 38.3 status to Done (file + index); updated `docs/TECHNICAL.md` with the liveness discipline (TUI 4-source event loop, web_fetch SSRF hardening, correction-loop subprocess bounds, worktree `spawn_blocking`).
...
### Fixed
- WO 38.10: CLI first-run + scriptability. (1) First-run banner moved to stderr — `--output stream-json` on a fresh data dir no longer emits a non-JSON banner line on stdout (P0). (2) Empty-model guard at session start: `default_model = ""` with no `-m` now exits 3 (ModelUnreachable) with a setup hint instead of the cryptic 400 from the OpenAI-compat fallback against Ollama (P0). (3) `run -p "<prompt>"` / `--prompt` one-shot flag: appends the arg as the first turn before the stdin loop, so `kf-code run -p "hello"` works without piping; a `-p` value is a single turn even with internal blank lines (fixes the multi-paragraph pipe truncation for the arg form) (P1). (4) Doc comments on the seven bare `run` flags (`model`, `host`, `model_type`, `auto_approve`, `system`, `resume`, `non_interactive`), including the denies-all note on `--non-interactive` (P1). (5) `NO_COLOR` decoupled from the TUI decision — only `TERM=dumb` / non-tty / `--no-tui` / `--non-interactive` / `--prompt` degrade to line mode; `NO_COLOR` now only suppresses colour/emoji in rendering, no longer kills the TUI (P1, spec violation). (6) Hard config.toml parse failure → exit 5 (ConfigParse) via the new `load_or_create_config_strict()` used on the `run`/`bench run` paths; the lenient `load_or_create_config()` keeps warn+defaults for plugin/legacy callers; unknown-key soft-merge warnings stay warnings (the documented exit 5 was previously unreachable) (P1). (7) `bench run` exit code reflects success: 0% pass (tasks_run > 0, tasks_passed == 0) now bails non-zero after writing the report, instead of exiting 0 unconditionally (P1). (8) `metrics_path()` honors `KF_CODE_DATA_DIR` so `kf-code metrics`/`verify` read the same installation `run` writes to (previously read the platform default on override) (P1). Deferred (disclosed in the workorder): `--read-stdin-full` escape hatch for piped-stdin blank-line semantics, `--output json` error-object emission on hard error, JSON summary `session.id`/`started_at` accuracy, and the P2 set.
...
### Changed
- WO 40.1 (CI workflow architecture reset): added `concurrency: { group: ci-${{ github.workflow }}-${{ github.ref }}, cancel-in-progress: false }` to `release.yml` — the one workflow still missing it. Items 2-7 of the spec (artificial `needs:` removal, `fmt`→`static` rename, PR fast-path split, path-aware conditional skip, redundant `cargo check` removal, incident-history comment cleanup) were already landed by the WO 33.x + ADR-074 work in this worktree's starting state; this WO audited them, confirmed them satisfied, and closed the lone concurrency gap. ci-pr/ci-merge keep per-PR isolation groups (`github.event.pull_request.number || github.ref`) and ci-nightly keeps `cancel-in-progress: false` per ADR-074.
