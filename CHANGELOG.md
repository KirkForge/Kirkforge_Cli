...
### Fixed
- WO 38.11: TUI state hygiene. (1) Chat render cache invalidated on CompactionReport, prune_display_messages, and fork-swap — `clear_entries()` at all 3 mutation sites (previously stale lines served at re-indexed positions). (2) `thinking_buffer` bounded to a 32 KiB tail budget (`trim_thinking_buffer_tail`); the render path no longer joins + re-wraps an unbounded buffer every frame. (3) Streaming PTY tool card capped to a 64 KiB tail with a byte-count marker (`… [N bytes total, showing last 64K]`); `watch`/`top`-style commands no longer balloon the card. (4) Chat scroll offset clamped to `u16::MAX` with a `ponytail:` ceiling note (ratatui 0.30 `Paragraph::scroll` takes u16; >65_535 wrapped lines wrapped the view to top). (5) Doom-banner keys routed above the approval dialog when unacknowledged — Esc now dismisses the banner instead of denying the tool call. (6) `TurnEvent::Error` clears stale `continuation`/`streaming` flags (mirrors `TurnComplete`). (7) Approval dialog diff memoized on `(tool_name, args JSON, side_by_side, dialog_width, file mtime)` — no more re-read + re-diff at 8 Hz. (8) `textwrap::fill` width-0 guarded with `.max(1)` at 3 sites (tiny panes no longer silently drop content).
