...
### Added
- WO 30.2: `TaskManager` lifecycle upgrade — `TaskStatus` (`Pending | Running | Completed | Cancelled | Failed | TimedOut`), per-task `TaskMetadata` (model, persona, ≤100-char prompt summary, started_at, duration_ms, token_estimate, parent_task_id), `cancel(id)` (stops a running subagent via a per-task cancel flag + `select!`), `status(id)`, and `list()` for the `/jobs` view. Status is derived from the existing `result`/`error` + flags, so the `task_output` tool and all existing callers are unchanged. DEFERRED: TUI `/jobs` rendering of subagent tasks — `TaskManager` is created per-toolset inside `all_tools()` and is not reachable from `AppState` (unlike the global `BashJobRegistry`); surfacing it needs a cross-layer decision (global singleton breaks per-session isolation vs a `ServicesState` field). Tracked in `state.md`. `format_task_entry` + `list()` are already in place so the wiring is a follow-up. Also fixes a pre-existing `cargo fmt` slip in `src/session/task_spawner.rs` from 5fbd955 (unrelated `.into()` line wrap).
...
- WO 30.6: Subagent approval forwarding — `task`-tool subagent destructive-tool approval requests are forwarded to the parent session's approval channel (set on the `InProcessTaskSpawner` from `Executor::run_turn`), so the user sees and decides them in the TUI / line-mode instead of the P0 deny-all. With no parent channel (top-level scheduled job) the P0 policy still applies (auto-approve in CI, deny otherwise). Nesting chains: each `run_task` pins its subagent executor's spawner to its own approval channel, so an N-deep subagent forwards level-by-level to the top-level user.
- WO 30.1: `coder` subagent worktree isolation — a `coder` subagent gets its own git worktree (via the existing `WorktreeSession::create`) and the path guard's `sandbox_dir` is pointed at it, so `read_file`/`write_file`/`edit_file`/`notebook_edit` land in an isolated checkout whose path is returned in the summary for the parent to review/merge. `explore`/`plan` keep reading the parent workspace. Bash CWD is NOT confined to the worktree (deferred — bash keeps its existing landlock/sandbox posture). Completed worktrees persist on disk for parent review (cleaned up on error via `Drop`).
...
- WO 30.6 (prerequisite fix): the executor built `self.task_spawner` (`build_task_spawner`) but `run_prepared_call` hardcoded `task_spawner: None` in the dispatch `ToolContext` and `PreparedCall` carried no spawner field — so the parent `task` tool always errored "task tool is not available in this context". The spawner is now threaded through `PreparedCall` → `ctx.task_spawner`; the field is held concretely (`Arc<InProcessTaskSpawner>`) so the executor can set the approval forwarder on it.
