...
### Fixed
- WO 38.12: test hygiene completion — killed remaining wall-clock margins in tests. (1) `shutdown_notify_wakes_select`: dropped the redundant 500ms elapsed bound (kept should_exit==true assert + 1s hang guard). (2) `test_mid_batch_checkpoint_persists_partial_results`: replaced the 150ms post-gate sleep with a `tokio::sync::watch` done-count gate (waits for 2 tool completions, no timing margin). (3) Three sleep-then-cancel tests (bash.rs, dispatch.rs, plugin_tools/wrapper.rs): replaced fixed-delay cancel with gated-start via readiness file (`touch ready; sleep 30` — poll for `ready` before firing cancel); no production readiness signal exists so the readiness file is a test-only technique. (4) `recent_list_is_capped`: replaced 10× `thread::sleep(10ms)` mtime stagger with explicit `set_modified` (std, no new dep). (5) `session_mode_round_trip` (both sites): added `SessionModeGuard` Drop guard to restore the prior mode on panic, complementing the existing `#[serial]` from WO 40.5. (6) `handle_jobs_command_cancel_running_job_succeeds`: un-ignored (root cause fixed in WO 36.2); replaced the 100ms sleep-then-cancel with `wait_for_job_running` poll-until-Running. CwdGuard (task 4) was already shipped by WO 40.5 — verified, not redone.
