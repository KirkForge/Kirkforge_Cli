...
### Changed
- WO 29.9: TS→Rust migration made permanent. Deleted the `npm/kf-plugin/` TS tree (~3.2 MB / ~16k LOC) and the dead `plugins/kf-plugin/` shell-plugin tree (its `tools/*.sh` scripts `exec node` against the deleted TS CLI). Removed the `Lint Node SDK (eslint)` CI step, the `ci-local.sh` Node SDK pass, the `build-all.sh` Node build path, and the `check-artifact-consistency.sh` plugin-dir check. `wrapper.rs::npm_bin_dirs()` simplified to only the data-dir layout (source-layout walk dropped). `kf-plugin` removed from `default_plugin_sources()` (compiled-in needs no filesystem source) and added to `default_enabled_plugins()` behind `kf-plugin-tools`. Dead tests deleted; pre-existing `folded_plugin_identification` bug fixed. Docs synced (TECHNICAL.md, config.toml.example, Cargo.toml feature comment, run_session.rs/loader.rs doc-comments). WO 28.4 marked superseded. Also resolves pre-existing unresolved merge markers in `Cargo.toml`/`Cargo.lock` (scope creep, mandatory — no gate ran without it).

### Skipped
- WO 29.8: Health-server port — skipped, not needed. The daemon is a Unix-socket session registry, not an HTTP server; no Kubernetes liveness or SLO-dashboard consumer exists for an HTTP health endpoint. Porting the TS health-server (axum/hyper + auth + rate-limit + Prometheus) would add a heavy HTTP framework to a size-optimized binary for no consumer.
...
### Fixed
- WO 27.2: Binary-spawn e2e scenarios (`tests/e2e/scenarios/`) un-hung. Two root causes: (1) `--non-interactive` startup ran the synchronous `kf-context-index` tree-sitter walk over the cwd, which is unbounded on large working trees (645 files took >90s in this repo) and wedged scripted startup — now skipped in non-interactive mode (interactive TUI runs keep the index). (2) The auto-started session daemon inherited the parent's piped stdout/stderr, so after the parent exited any piped caller (CI, shell pipelines, the test harness) hung forever on `read_to_end`; `start_daemon` and `daemonize` now detach stdio to `/dev/null`. The e2e harness also seeds `[adapter_routing] "e2e-" = "Ollama"` (mirrors the in-process test) and bounds its post-exit pipe drain with a 5s deadline so any future fd-inheritance regression surfaces as a named `TimedOut` instead of an infinite hang. All 7 previously-`#[ignore]`'d scenarios now pass (~5s total). DEFERRED: the underlying `index_dir` pathological slowness on large repos (gap #27) is masked, not fixed — interactive TUI users still pay it; tracked in `state.md` pending.
