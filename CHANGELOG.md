...
### Fixed
- WO 30 (Python pipeline audit): the pytest verifier (`src/session/verifier/python_test.rs`) spawned `python -m pytest`, which is `NotFound` on python3-only hosts (most Linux distros ship NO `python` symlink), so the verifier ALWAYS returned `Verdict::Unfixable("failed to spawn python")` and never ran pytest. Added `pick_python()` (probes `python3` then `python` via `--version`, mirroring `python_lint::pick_linter`) and the verifier now resolves the real interpreter, skipping gracefully only when neither exists. 2 new tests. (WO 30.0.9 only added a bash-tool DESCRIPTION note about the symlink gap — it never fixed the verifier itself.)
- WO 30 (Python pipeline audit): re-fixed a WO 30.0.8 regression — `read_file` auto-minify was firing even when `minify_write_side=false`. The `minify_write_side` gate documented in WO 30.0.8 (and the `auto_minify_skipped_when_write_side_disabled` test) had regressed out of the code (`auto_minified` no longer checked the flag), leaving that test RED at HEAD. Restored the one-line gate so auto-minify fires only when `minify_write_side=true` (the envelope then round-trips through `edit_file`); without it the model received PLAIN minified text that can't match `edit_file`'s raw-string compare, stalling edits.
- WO 30 (Python pipeline audit, disclosed scope creep): pre-existing gate-blockers in `src/tui/widgets/chat/` from the last commit (`4668f91`) — `cargo fmt --check` drift + two clippy `-D warnings` violations (`manual_div_ceil` → `div_ceil`, `uninlined_format_args`). Mechanical `cargo fmt` + `cargo clippy --fix`; no logic change. Fixed because the audit's gate could not be green otherwise.
