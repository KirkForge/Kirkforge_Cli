//! `kf-budget budget {status,set,reset,compact}` — inspect or set the
//! token budget ceiling, reset the session counter, and emit a
//! compact hint.

use kf_budget_core::{budget::BudgetConfig, compaction};
...
// ponytail: B2 fix — `kf-budget budget reset` zeros `used` while
// preserving `ceiling` and `approaching_ratio`. The previous gap
// (kf-budget-gaps.md § B2) was that `used` lives in `budget.toml`
...
// `kf-budget budget set --default 250000` keeps that ceiling.
...
mod compact_tests {
    use super::*;
    use kf_budget_core::budget::TokenBudget;
    use kf_budget_core::cost::{UsageKind, UsageRecord};
    ...
    // ponytail: pin the `kf-budget budget compact --json` wire shape
    ...
    // ponytail: pin the wire shape of `kf-budget budget status --json`.
    ...
    // ponytail: pin the JSON wire shape of `kf-budget budget reset --json`.
    ...
    fn reset_persists_zeroed_used_to_disk() {
        ...
        // ponytail: env-var guard lives in `kf_budget_core::test_support`
        ...
        let seeded = kf_budget_core::budget::TokenBudget {
            ceiling: 250_000,
            approaching_ratio: 0.7,
            used: 12_345,
        };
        ...
        let pre: kf_budget_core::budget::TokenBudget =
        ...
        let post: kf_budget_core::budget::TokenBudget =
