//! `kf-budget config {show,validate}` — print effective config
//! paths or write-test them (ADR-0015 § Validate).

use kf_budget_core::{budget::ConfigFile, budget::TokenBudget, Paths};
...
pub(crate) fn validate(as_json: bool) {
    ...
    // failure → 78 (EX_CONFIG). A host that polls `kf-budget config
...
mod tests {
    ...
    // The whole point of `kf-budget config --validate` is to
    ...
    // a user who runs `kf-budget config --validate` on a brand
    ...
    // at the helper boundary). The whole point of `kf-budget config
