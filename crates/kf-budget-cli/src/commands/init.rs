//! `kf-budget init` — write the host's hook entries into the host's
...
use kf_budget_core::atomic_write_text;
use kf_budget_hosts::Host;
...
pub(crate) struct MergeOutcome {
    ...
    pub merged: Value,
    /// `true` if at least one of kf-budget's hook entries replaced
    /// an existing entry (timeout drift, command drift, etc.).
    pub updated_own: bool,
    /// `true` if a non-kf-budget hook entry was preserved on a slot
...
/// the caller authorised overwriting conflicting kf-budget hooks,
...
///    NOT start with `"kf-budget "`. The user added those and we
///    do not own them.
/// 3. Replace any `kf-budget ` entry on a slot we also write to.
...
///    an existing `kf-budget ` entry with a *different* command on
...
pub(crate) fn merge_into_settings(
    ...
    // For each slot kf-budget owns, build the merged array.
    for slot in ["PostToolUse", "UserPromptSubmit", "PreCompact"] {
        ...
        // command does NOT start with "kf-budget ".
        ...
        if let Some(existing_slot) = hooks_obj.get(slot).and_then(|v| v.as_array()) {
            for entry in existing_slot {
                let is_ours = entry
                    .get("command")
                    .and_then(|c| c.as_str())
                    .is_some_and(|c| c.starts_with("kf-budget "));
                if is_ours {
                    // Existing kf-budget entry. Same command? OK to
        ...
        if !merged_array.is_empty() {
            ...
            // we now want to leave them gone (e.g. kf-budget trimmed
...
pub(crate) enum MergeError {
    ...
    NotAnObject,
    /// A `kf-budget ` hook was found with a different `command` on
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            MergeError::Conflict { slot, existing_cmd } => write!(
                f,
                "settings file already has a `kf-budget ` entry on {slot:?} \
...
/// a per-host `Paths` extension in kf-budget-core (the XDG-style
/// resolution is kf-budget-specific, not host-specific), so this
/// lives in the only consumer (`commands::init`) rather than a
/// hypothetical `kf-budget-hosts/src/claude_code.rs` slot.
...
///   existing file already has the same kf-budget entries)
/// - 1  = bad host argument (host has no settings-file schema yet)
/// - 2  = settings dir is not creatable
/// - 3  = conflict (existing kf-budget hook with a different command)
...
pub(crate) fn run(host: Host, dry_run: bool, force: bool, as_json: bool) -> i32 {
    let home = if let Some(h) = std::env::var_os("HOME") {
        ...
        eprintln!("kf-budget init: HOME is not set; cannot resolve settings path");
        return 4;
    };
    let path = match host {
        ...
        // other four so a user who runs `kf-budget init` on a non-
        ...
        _ => {
            eprintln!(
                "kf-budget init: host {host:?} has no settings-file schema yet \
    ...
    let existing = match std::fs::read_to_string(&path) {
        ...
        Ok(s) => match serde_json::from_str::<Value>(&s) {
            ...
            Err(e) => {
                eprintln!(
                    "kf-budget init: settings file at {} is not valid JSON: {e}",
                    path.display()
                );
                return 4;
            }
        ...
        Err(e) => {
            eprintln!("kf-budget init: read {} failed: {e}", path.display());
            return 4;
        }
    };
    ...
    let hook_json = match serde_json::to_value(register_hooks_for(host)) {
        Ok(v) => v,
        Err(e) => {
            eprintln!("kf-budget init: HookConfig serialise failed: {e}");
            return 4;
        }
    };
    ...
    let outcome = match merge_into_settings(existing.as_ref(), &hook_json, force) {
        ...
        Err(MergeError::NotAnObject) => {
            eprintln!("kf-budget init: {}", MergeError::NotAnObject);
            return 4;
        }
        Err(MergeError::Conflict { slot, existing_cmd }) => {
            eprintln!("kf-budget init: conflict on {slot}: existing command = {existing_cmd}");
            return 3;
        }
    };
    ...
    if let Some(parent) = path.parent() {
        if let Err(e) = std::fs::create_dir_all(parent) {
            eprintln!("kf-budget init: cannot create {}: {e}", parent.display());
            return 2;
        }
    }
    ...
    let pretty = match serde_json::to_string_pretty(&outcome.merged) {
        Ok(s) => s,
        Err(e) => {
            eprintln!("kf-budget init: serialise failed: {e}");
            return 4;
        }
    };
    ...
    // any concurrent writer would be a `kf-budget init` racing
    ...
    match std::fs::read_to_string(&path) {
        ...
        Ok(_) => {
            eprintln!(
                "kf-budget init: post-write verify failed at {}",
                path.display()
            );
            return 4;
        }
        Err(e) => {
            eprintln!(
                "kf-budget init: post-write read {} failed: {e}",
                path.display()
            );
            return 4;
        }
...
mod tests {
    ...
    fn kf-budget_hooks() -> Value {
        ...
        json!({
            "PostToolUse": [{
                "type": "command",
                "command": "kf-budget hook post-tool-use",
                "timeout": 5,
            }],
            "UserPromptSubmit": [{
                "type": "command",
                "command": "kf-budget hook user-prompt-submit",
                "timeout": 2,
            }],
            "PreCompact": [{
                "type": "command",
                "command": "kf-budget hook pre-compact",
                "timeout": 10,
            }],
        })
    }
    ...
    fn merge_preserves_user_added_hook_on_same_slot() {
        ...
        // Our hook is SECOND.
        assert_eq!(arr[1]["command"], "kf-budget hook post-tool-use");
        assert!(
            outcome.preserved_foreign,
    ...
    // already has `kf-budget hook post-tool-use` from a prior
    ...
    fn merge_replaces_existing_kf-budget_entry_with_same_command() {
        ...
        assert_eq!(
            arr.len(),
            1,
            "existing kf-budget entry must be replaced, not duplicated; got {arr:?}"
        );
        assert_eq!(
            arr[0]["timeout"], 5,
            "replacement must use the new timeout from register_hooks"
        );
        assert_eq!(arr[0]["command"], "kf-budget hook post-tool-use");
    ...
    // `kf-budget ` entry with a DIFFERENT command (not the one we
    ...
    fn merge_refuses_different_kf-budget_command_without_force() {
        ...
        let existing = json!({
            "hooks": {
                "PostToolUse": [{
                    "type": "command",
                    "command": "kf-budget hook post-tool-use --variant=foo",
                    "timeout": 5,
                }],
            },
        });
    ...
    fn merge_with_force_accepts_different_kf-budget_command() {
        ...
        assert_eq!(
            arr[0]["command"], "kf-budget hook post-tool-use",
    ...
    // file that already has every kf-budget entry with the
    ...
    fn merge_with_existing_identical_kf-budget_entries_is_stable() {
        ...
        assert!(
            outcome.updated_own,
            "existing kf-budget entry → updated_own=true (replaced by the same entry)"
        );
    }
    ...
    // who switches to `~/.config/kf-budget/settings.json` (the
    ...
    // documented in kf-budget-gaps.md). Sequential sub-scopes
    ...
    fn run_io_wrapper_end_to_end_scenarios() {
        ...
        // ---- Scenario 3: existing conflict → exit 3, --force → exit 0 ----
        {
            ...
            assert_eq!(
                parsed["hooks"]["PostToolUse"][0]["command"], "kf-budget hook post-tool-use",
