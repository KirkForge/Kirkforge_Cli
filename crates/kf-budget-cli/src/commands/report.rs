//! `kf-budget report` — query cost-reporting records. ADR-0010.
//!
//! ponytail: this module is now a thin shell. The pure aggregator
//! (`aggregate_sessions`, `filter_lines`, `tail_lines`) lives in
//! kf-budget-core where it can be tested without the binary. The
//! CLI side parses clap args and prints.

use kf_budget_core::{cost::UsageKind, report, Paths};
...
// ponytail: path-parameterised `at` so tests in kf-budget-core
...
pub(crate) fn at(
    ...
    let Ok(s) = std::fs::read_to_string(path) else {
        ...
        // stdout; a `kf-budget --json report` on a clean XDG data
        ...
        if as_json {
            ...
            eprintln!("kf-budget: no usage.jsonl at {}", path.display());
        }
        return 0;
    };
...
mod tests {
    ...
    // compact (mirrors kf-budget-core/src/report.rs::tests::rec).
    ...
    // wrapper script that pipes `kf-budget --json report` into jq.
    #[test]
