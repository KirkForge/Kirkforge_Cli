//! `kf-budget store {prune,get}` — manage the offload store: evict
...
pub(crate) fn prune(as_json: bool) {
    ...
    // markers (`<<kf-budget:slice:abc...>>`); only the markers map
...
// ponytail: B5 fix — `kf-budget store get <marker>` is the
...
pub(crate) fn get(marker: &str, as_json: bool) -> i32 {
    ...
    // `<<kf-budget:slice:` and end with `>>` — the wire format the
    ...
    // marker like `<<kf-budget:slice:!!>>` would yield key="!!" — which
    ...
    match store.get(key) {
        Ok(bytes) => {
            if as_json {
                ...
                // (`kf-budget store get <m> | jq .`). The JSON path
...
mod tests {
    ...
    fn key(b: &[u8]) -> String {
        kf_budget_core::store::make_key(b)
    }
    fn marker(b: &[u8]) -> String {
        kf_budget_core::store::format_slice_marker(&key(b))
    }

    // ponytail: pin the base64 encoder end-to-end. The
    // `--json` envelope on `kf-budget store get` calls this with
    ...
    // (e.g. to "<<kf-budget:" for all kinds) surfaces here.
    #[test]
    fn parse_slice_marker_round_trip_with_format() {
        let k = key(b"alpha");
        let m = kf_budget_core::store::format_slice_marker(&k);
        assert_eq!(
    ...
    // ponytail: integration pin for `kf-budget store get <marker>`
    ...
    // A host calls `kf-budget store get <marker>` and expects the
    ...
    fn store_get_round_trips_through_disk() {
        ...
        let _g = kf_budget_core::test_support::EnvGuard::set("PLUGIN3_DATA_DIR", dir_str);
        ...
        // some crates, but kf-budget-cli doesn't depend on it.
        ...
        let bogus_marker = kf_budget_core::store::format_slice_marker("deadbeefdeadbeefdeadbeef");
    ...
    // of a recent key" surfaces here (would let `<<kf-budget:slice:`
    ...
    // `kf-budget store prune --json` reports `removed` and `kept`.
