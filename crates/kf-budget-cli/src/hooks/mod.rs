...
//! Per ADR-0002 § Crate layout (`crates/kf-budget-cli/src/hooks/`).
//!
...
fn print_json<T: Serialize>(value: &T, fallback: &str) {
    let s = serde_json::to_string(value).unwrap_or_else(|e| {
        eprintln!("kf-budget: response serialisation failed: {e}");
        fallback.to_string()
    });
    println!("{s}");
}
...
// so `kf-budget init` (a future subcommand) writes the JSON, not
    ...
    match host {
        Host::ClaudeCode => HookConfig {
            post_tool_use: Some(vec![CommandHook {
                kind: "command",
                command: "kf-budget hook post-tool-use".into(),
                timeout: 5,
            }]),
            user_prompt_submit: Some(vec![CommandHook {
                kind: "command",
                command: "kf-budget hook user-prompt-submit".into(),
                timeout: 2,
            }]),
            pre_compact: Some(vec![CommandHook {
                kind: "command",
                command: "kf-budget hook pre-compact".into(),
                timeout: 10,
            }]),
        },
...
pub(crate) fn post_tool_use() {
    ...
    let Some(payload) = read_stdin_json::<PostToolUsePayload>() else {
        ...
        let resp = PostToolUseResponse {
            content: String::new(),
            note: Some("kf-budget: stdin parse failed; passing through".into()),
        };
        print_json(
            &resp,
            r#"{"content":"","note":"kf-budget: response serialisation failed"}"#,
        );
        return;
    };
    ...
    let Some((_, decision)) = result.decisions.into_iter().next() else {
        ...
        eprintln!("kf-budget: orchestrator returned no decision; passing input through");
        let resp = PostToolUseResponse {
            content: payload.content.clone(),
            note: Some("kf-budget: no slicing decision produced; passing through".into()),
        };
        print_json(
            &resp,
            r#"{"content":"","note":"kf-budget: response serialisation failed"}"#,
    ...
    // which inflated `records` and the `kf-budget report --kind slice`
...
pub(crate) fn user_prompt_submit() {
    ...
    // must not inflate the warnings count in `kf-budget report
    ...
    // `UserPromptSubmitResponse` (kf-budget-hosts) are byte-equivalent
