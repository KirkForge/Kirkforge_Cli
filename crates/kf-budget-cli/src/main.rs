...
//! kf-budget CLI — host hooks + budget + cost reporting.
...
enum Command {
    ...
    /// Manage the offload store (B4 fix, kf-budget-gaps.md).
    Store {
        #[command(subcommand)]
        sub: StoreSub,
    },
    /// Write the host's hook entries into the host's settings
    /// file (B9 fix, kf-budget-gaps.md; ADR-0009).
    Init {
        ...
        #[arg(long)]
        dry_run: bool,
        /// Overwrite an existing `kf-budget ` hook with a different
...
enum StoreSub {
    ...
    Get {
        /// The `<<kf-budget:slice:...>>` marker from the Slice response.
        marker: String,
    },
}

// ponytail: clap-side mirror of `kf_budget_hosts::Host`.
// `Host` is a typed enum but lacks the `clap::ValueEnum` derive
// (kf-budget-hosts has no clap dep). Mirroring here keeps the
// host registry decoupled from the CLI's arg parser — a
// contributor who adds `Host::Codex` to kf-budget-hosts adds a
...
impl From<HostArg> for kf_budget_hosts::Host {
    fn from(a: HostArg) -> Self {
        match a {
            HostArg::ClaudeCode => kf_budget_hosts::Host::ClaudeCode,
            HostArg::Cursor => kf_budget_hosts::Host::Cursor,
            HostArg::Aider => kf_budget_hosts::Host::Aider,
        }
    }
}
...
// these into `kf-budget hook --help` and a `--json` self-check
...
enum BudgetSub {
    ...
    /// `approaching_ratio` are preserved (B2 fix, kf-budget-gaps.md).
    Reset,
    ...
    match cli.command {
        ...
        Command::SelfCheck => {
            if let Err(e) = self_check() {
                eprintln!("kf-budget self-check failed: {e}");
                std::process::exit(1);
            }
        }
        Command::Config {
        ...
        // ponytail: B9 fix — `kf-budget init` writes the host's
...
// `crates/kf-budget-cli/src/hooks/`. The three `run_*` functions
...
fn self_check() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let key = kf_budget_core::parse_slice_marker(marker)
    ...
    println!("kf-budget self-check OK (slicing + budget + offload round-trip)");
    Ok(())
}
