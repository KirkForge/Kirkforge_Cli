...
//! config file, project-local `.kf-budget/config.toml`) adds another
//! `if let Some(...)` arm.
...
mod tests {
    ...
    fn xdg() -> PathBuf {
        PathBuf::from("/home/u/.config/kf-budget")
    }
    ...
    fn cli_wins_over_xdg() {
        ...
        let cli = Some(Path::new("/etc/kf-budget/config.toml"));
        let got = resolve_config_path(cli, &env, &xdg());
        assert_eq!(got, PathBuf::from("/etc/kf-budget/config.toml"));
    }
    ...
    fn xdg_default_used_when_neither_cli_nor_env_set() {
        ...
        let got = resolve_config_path(None, &env, &xdg());
        assert_eq!(got, PathBuf::from("/home/u/.config/kf-budget/config.toml"));
    }
