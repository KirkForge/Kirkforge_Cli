...
//! `kf-budget-cli/src/main.rs` clap impl, the `Command` enum,
...
// live in `crates/kf-budget-cli/src/main.rs`, not in a
...
fn adr_0015_top_level_block_points_to_main_rs() {
    let block = adr_0015_top_level_block();
    assert!(
        block.contains("crates/kf-budget-cli/src/main.rs"),
        "ADR-0015 § Top-level structure example must \
         reference `crates/kf-budget-cli/src/main.rs` — the \
...
// `--version` flag handles `kf-budget --version`; a custom
...
// filters `kf-budget report --kind`. The drift test
...
fn adr_0015_impl_directory_layout_matches_adr() {
    let cli_root = repo_root().join("crates").join("kf-budget-cli").join("src");
