...
//! 4. `precedence.rs` module docstring (crates/kf-budget-cli/src/precedence.rs)
...
fn precedence_rs_module_docstring_names_cli_first() {
    let src = read(&repo_root().join("crates/kf-budget-cli/src/precedence.rs"));
...
fn adr_0002_file_tree_annotation_names_cli_first() {
    ...
    // near the `precedence.rs` entry under `kf-budget-cli/src/`.
