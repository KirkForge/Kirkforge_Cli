...
//! `kf-budget-core/src/compaction.rs` impl and the
//! `kf-budget-cli/src/main.rs` clap subcommand. Companion to the
...
// a future-ADR retrofit for the `kf-budget budget set
...
fn adr_0008_compact_subcommand_block_lists_three_variants() {
    ...
    assert!(
        ...
        "ADR-0008 § Compact subcommand example must declare \
         ...
         `BudgetSub::Set {{ ceiling, default }}` arms `kf-budget \
