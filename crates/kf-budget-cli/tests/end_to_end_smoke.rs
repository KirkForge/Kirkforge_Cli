...
//! every assertion runs against the actual `kf-budget` CLI binary
...
fn end_to_end_post_tool_use_budget_report_round_trip() {
    ...
    assert!(
        ...
        "sliced output must carry the kf-budget marker; got: {content:?}"
    );
    assert!(
...
fn end_to_end_budget_guard_forces_slice_on_oversized_prompt() {
    ...
    // (`<<kf-budget:slice:hash>>`), not `tool_result_key`. The budget
    ...
    assert!(
        target_key.starts_with("<<kf-budget:slice:"),
...
/// `kf-budget config --validate` end-to-end — pins the exit-code
...
/// `kf-budget config --validate --json` end-to-end — pins the
