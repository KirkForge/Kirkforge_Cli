...
//! `kf-budget-cli/src/hooks/mod.rs` impl. Companion to the
        ...
        if let Some(fence_end_rel) = fence_after.find("```") {
            ...
            assert!(
                ...
                "ADR-0009 § Error contract code block must not \
                 ...
                 emits one `eprintln!` line tagged `kf-budget:` to \
                 the host's stderr.",
            );
        }
    }
...
fn adr_0009_implementation_notes_json_example_pins_wire_fields() {
    ...
    assert!(
        block.contains("\"kf-budget hook post-tool-use\"")
            && block.contains("\"kf-budget hook user-prompt-submit\"")
            && block.contains("\"kf-budget hook pre-compact\""),
        "ADR-0009 § Implementation notes JSON example must show the \
         three kf-budget hook command strings.",
    );
}
...
// free function exported from `kf_budget_core` as
...
fn adr_0009_post_tool_use_flow_uses_run_orchestrator_free_fn() {
    ...
    assert!(
        ...
        "ADR-0009 § PostToolUse flow code block must call \
         ...
         exported as `kf_budget_core::orchestrator::run`. The impl \
...
// uses `decide` from `kf_budget_core::budget`. A contributor
...
fn adr_0009_user_prompt_submit_flow_calls_decide() {
    ...
    assert!(
        ...
        "ADR-0009 § UserPromptSubmit flow code block must call \
         ...
         exported as `kf_budget_core::budget::decide`. The impl \
...
// `kf-budget report --kind slice` count. The aggregator's
...
fn adr_0009_post_tool_use_flow_gates_emit_on_sliced_decision() {
    ...
    assert!(
        ...
        "ADR-0009 § PostToolUse flow code block must wrap the \
         ...
         `kf-budget report --kind slice`. The orchestrator invariant \
