...
// the warnings count in `kf-budget report --summary` (a regression
...
pub(crate) fn emit_usage_at(record: &UsageRecord, path: &std::path::Path) {
    ...
    if let Err(e) = writeln!(file, "{line}") {
        tracing::warn!("kf-budget: usage.jsonl write failed ({e}); dropping record");
    }
}
...
mod tests {
    ...
    fn classify_kind_allow_returns_none() {
        ...
        // `kf-budget report --summary`. Removing this None makes the
