...
mod tests {
    ...
    fn marker_full_shape_end_to_end() {
        ...
        let m = format_slice_marker(&k);
        assert!(
            m.starts_with("<<kf-budget:slice:"),
            "marker must start with kf-budget prefix, got: {m:?}"
        );
        assert!(
            m.ends_with(">>"),
    ...
    fn marker_parse_rejects_near_misses() {
        ...
        assert_eq!(parse_slice_marker("<<kf-budget:slice:abc>>missing"), None);
        // Single > instead of >>.
        assert_eq!(parse_slice_marker("<<kf-budget:slice:abc>"), None);
        // Wrong prefix.
        assert_eq!(parse_slice_marker("<<kf-budgetv2:slice:abc>>"), None);
        ...
        assert_eq!(parse_slice_marker("<<kf-budget:slice:>>"), Some(""));
    }
