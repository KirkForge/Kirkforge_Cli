//! Shared test helpers for kf-budget-core.
//!
//! This module is compiled into every build so that downstream crate
//! tests (e.g. `kf-budget-cli`) can reuse the helpers. It is intended
...
// env-var mutation made by kf-budget-core tests. `std::env::set_var`
// is process-global; parallel `#[test]` runs read each other's
// overrides and produce flaky failures (kf-budget-gaps.md § B8). A
