...
//! `kf-budget-core/src/store.rs` impl. Companion to
...
// <marker>`, and `grep -F '<<kf-budget:slice:'` is the
...
fn adr_0004_marker_block_pins_literal_prefix_and_suffix() {
    ...
    assert!(
        block.contains("\"<<kf-budget:slice:\""),
        "ADR-0004 § Marker format block must pin SLICE_MARKER_PREFIX as \
         `\"<<kf-budget:slice:\"` to match `store.rs`. Got:\n{block}",
    );
    assert!(
        block.contains("\"\\\"<<kf-budget:slice:\"") || block.contains("\"<<kf-budget:slice:\""),
    ...
    // marker block must contain the literal `<<kf-budget:slice:`
    ...
    assert!(
        block.contains("<<kf-budget:slice:") && block.contains(">>"),
        "ADR-0004 § Marker format block must contain the literal \
         `<<kf-budget:slice:` prefix and `>>` suffix strings — they are \
...
fn adr_0004_implementation_path_is_store_rs() {
    ...
    assert!(
        ...
        "ADR-0004 § Implementation notes must reference \
         `crates/kf-budget-core/src/store.rs` (single-file layout) — \
...
// `crates/kf-budget-core/tests/store_drift.rs`. The actual
...
fn adr_0004_implementation_path_drift_test_exists() {
    let path = repo_root().join("crates/kf-budget-core/tests/store_drift.rs");
    assert!(
        path.is_file(),
        "ADR-0004 § Implementation notes says the drift test lives at \
         `crates/kf-budget-core/tests/store_drift.rs` but the file is \
...
// The MVP's `open_store` helper in `crates/kf-budget-cli/src/
