...
/// (`<<kf-budget:slice:{}>>`) in negation context, and the
...
// `format!("<<kf-budget:slice:{}>>", key)` — a contributor
...
fn adr_0003_head_tail_slicer_block_uses_format_slice_marker() {
    ...
    assert!(
        ...
        "ADR-0003 § HeadTailSlicer code block must not contain \
         the inline `<<kf-budget:slice:{{}}>>` format string — the \
...
// helper emits one `eprintln!` line tagged `kf-budget:` and
...
fn adr_0003_slice_or_skip_omits_tracing_warn() {
    ...
    assert!(
        ...
        "ADR-0003 § Implementation notes `slice_or_skip` code \
         ...
         `kf-budget:` so the host's stderr captures the regression.",
    );
    ...
    assert!(
        ...
        "ADR-0003 § Implementation notes `slice_or_skip` code \
         ...
         `kf-budget: slicer failed; passing input through: {{e}}` \
