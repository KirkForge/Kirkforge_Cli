...
fn release_binary_path() -> std::path::PathBuf {
    ...
    // is `crates/kf-budget-core/tests/...` → workspace root is three
    // levels up; from there `target/release/kf-budget` is the standard
    // cargo location.
    let manifest = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    manifest
        .ancestors()
        .nth(2) // crates/kf-budget-core -> crates -> workspace root
    ...
    let Some(size) = release_binary_size() else {
        eprintln!(
            "skipping: release binary not built at {}; \
             run `cargo build --release --bin kf-budget` to enforce the budget.",
