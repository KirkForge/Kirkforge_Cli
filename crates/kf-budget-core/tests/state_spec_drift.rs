...
//! `kf-budget-core/src/paths.rs`, `kf-budget-core/src/atomic_write.rs`,
//! and `kf-budget-cli/src/main.rs` impls. Companion to
...
// ADR-0011), and a `lock` file in `$XDG_RUNTIME_DIR/kf-budget/`
...
fn adr_0014_directory_layout_block_does_not_claim_phantom_entries() {
    ...
    // B2: budget.toml now lives under $XDG_RUNTIME_DIR/kf-budget/, so
...
fn adr_0014_path_resolution_block_uses_paths_rs() {
    ...
    assert!(
        ...
        "ADR-0014 § Path resolution code block must reference \
         `crates/kf-budget-core/src/paths.rs` — that is where the \
         `Paths` struct lives. The earlier `state.rs` reference is \
         phantom; no such module exists in `kf-budget-core`.",
    );
    assert!(
...
// `crates/kf-budget-core/src/state.rs`. The split that
...
// log lines (`kf-budget: recent dir create failed: ...`).
