...
//! `kf-budget-core/src/budget.rs` impl. Companion to the in-file
...
use kf_budget_core::budget::{BudgetConfig, ConfigFile, UsageConfig};
...
// from `kf-budget-core` directly — the two tagged enums
...
fn adr_0005_user_prompt_submit_block_serialises_intervention_directly() {
    ...
    assert!(
        ...
        "ADR-0005 § UserPromptSubmit hook flow example must \
         ...
         `crates/kf-budget-cli/src/hooks/mod.rs::user_prompt_submit` \
