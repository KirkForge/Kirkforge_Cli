...
//! `kf-budget-core/src/detector.rs` impl and the `DetectorCache`
...
// in `crates/kf-budget-core/src/orchestrator.rs` (not
// `crates/kf-budget-core/src/detector/cache.rs`). A
...
fn adr_0006_detector_caching_block_points_to_orchestrator_module() {
    let block = adr_0006_detector_caching_block();
    assert!(
        block.contains("crates/kf-budget-core/src/orchestrator.rs"),
        "ADR-0006 § Detector caching example must point to \
         `crates/kf-budget-core/src/orchestrator.rs` — the \
...
fn adr_0006_implementation_notes_point_to_orchestrator_module() {
    ...
    assert!(
        ...
        "ADR-0006 § Implementation notes must reference the \
         ...
         `DetectorCache` is in `crates/kf-budget-core/src/orchestrator.rs`.",
    );
    // Negative: the detector/cache.rs path must NOT appear.
    assert!(
