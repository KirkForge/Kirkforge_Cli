...
pub fn detect_host_with(env: &dyn EnvSource) -> Host {
    ...
    // Precedence: CLAUDE_CODE > CURSOR_TRACE_ID > AIDER > KF_CODE_PLUGIN3 > ClaudeCode.
    ...
    if env.is_set("CLAUDE_CODE") {
        ...
        Host::Aider
    } else if env.is_set("KF_CODE_PLUGIN3") {
        Host::KfCode
    } else {
        Host::ClaudeCode
    }
...
mod tests {
    ...
    // `Intervention` lives in `kf-budget-core::budget::tests` — both
    ...
    fn user_prompt_submit_response_wire_shape_pins_all_four_variants() {
        ...
        // (see kf-budget-core::budget::tests::compact_reason_string_format_is_pinned).
    ...
    // KF_CODE_PLUGIN3 > default-to-ClaudeCode) is load-bearing:
