...
use kf_draw_core::{text_util, DrawState, Point};
...
pub struct FindState {
    pub query: String,
    pub matches: Vec<kf_draw_core::TextMatch>,
    pub index: usize,
}
...
pub struct MarqueeState {
    pub anchor: Point,
    pub current: Point,
    pub mode: kf_draw_core::SelectionMode,
}
...
impl App {
    ...
    pub fn find_insert(&mut self, ch: char) {
        ...
        f.query.push(ch);
        f.matches = kf_draw_core::find_matches(&self.state, &f.query);
        f.index = 0;
    ...
    pub fn cycle_find(&mut self) {
        ...
        if self.state.select_id(&m.id) {
            let field = match m.field {
                kf_draw_core::MatchField::Id => "id",
                kf_draw_core::MatchField::Content => "content",
            };
            self.status = format!(
