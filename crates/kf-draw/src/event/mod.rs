...
use kf_draw_core::{hit_test_box_handles, DrawObject, Point, Rect};

use kf_draw_core::save_document;

use kf_draw_core::DrawMode;
use kf_draw_core::{filter_palette, PaletteAction};
...
fn handle_key(app: &mut App, key: KeyEvent) {
    ...
    match key.code {
        ...
        KeyCode::Char(c) if (c == 'l' || c == 'L') && ctrl && shift && !alt => {
            align_selection(app, kf_draw_core::Align::Left);
        }
        KeyCode::Char(c) if (c == 'r' || c == 'R') && ctrl && shift && !alt => {
            align_selection(app, kf_draw_core::Align::Right);
        }
        KeyCode::Char(c) if (c == 't' || c == 'T') && ctrl && shift && !alt => {
            align_selection(app, kf_draw_core::Align::Top);
        }
        KeyCode::Char(c) if (c == 'b' || c == 'B') && ctrl && shift && !alt => {
            align_selection(app, kf_draw_core::Align::Bottom);
        }
        KeyCode::Char(c) if (c == 'h' || c == 'H') && ctrl && shift && !alt => {
            align_selection(app, kf_draw_core::Align::HorizontalCenter);
        }
        KeyCode::Char(c) if (c == 'v' || c == 'V') && ctrl && shift && !alt => {
            align_selection(app, kf_draw_core::Align::VerticalCenter);
        }
        ...
        KeyCode::Char(c) if (c == 'j' || c == 'J') && ctrl && shift && !alt => {
            distribute_selection(app, kf_draw_core::DistributeAxis::Horizontal);
        }
        KeyCode::Char(c) if (c == 'k' || c == 'K') && ctrl && shift && !alt => {
            distribute_selection(app, kf_draw_core::DistributeAxis::Vertical);
        }
...
/// Code convention.
fn mode_from_modifiers(mods: KeyModifiers) -> kf_draw_core::SelectionMode {
    use kf_draw_core::SelectionMode;
...
/// path.
fn marquee_rect(a: Point, b: Point) -> kf_draw_core::Rect {
    kf_draw_core::Rect {
        left: a.x.min(b.x),
        top: a.y.min(b.y),
        right: a.x.max(b.x),
...
fn handle_layer_click(
    ...
    let layers = kf_draw_core::layer_list(&app.state);
    ...
    match mode {
        ...
        kf_draw_core::SelectionMode::Add => {
        ...
        kf_draw_core::SelectionMode::Toggle => {
...
fn cycle_layer_focus(app: &mut App, delta: i32) {
    ...
    app.status = format!(
        ...
        next + 1,
        n,
        kf_draw_core::kind_label(layer.kind),
        layer.id
    );
}
...
fn hit_test_selected_box(
    state: &kf_draw_core::DrawState,
    point: kf_draw_core::Point,
    tolerance: i32,
) -> Option<kf_draw_core::BoxResizeHandle> {
    ...
    let kf_draw_core::DrawObject::Box(b) = sel[0] else {
        return None;
    };
    let r = Rect {
        left: b.left,
...
fn dispatch_palette_action(app: &mut App, action: PaletteAction, name: &str) {
    match action {
        ...
        PaletteAction::AlignLeft => {
            palette_align(app, name, kf_draw_core::Align::Left);
        }
        PaletteAction::AlignRight => {
            palette_align(app, name, kf_draw_core::Align::Right);
        }
        PaletteAction::AlignTop => {
            palette_align(app, name, kf_draw_core::Align::Top);
        }
        PaletteAction::AlignBottom => {
            palette_align(app, name, kf_draw_core::Align::Bottom);
        }
        PaletteAction::AlignHorizontalCenter => {
            palette_align(app, name, kf_draw_core::Align::HorizontalCenter);
        }
        PaletteAction::AlignVerticalCenter => {
            palette_align(app, name, kf_draw_core::Align::VerticalCenter);
        }
...
fn palette_align(app: &mut App, name: &str, how: kf_draw_core::Align) {
    align_selection(app, how);
    app.status = format!("palette: {name} → {}", app.status);
}

fn ink_color_for_digit(digit: char) -> kf_draw_core::InkColor {
    use kf_draw_core::InkColor;
    match digit {
        '1' => InkColor::White,
        '2' => InkColor::Red,
        '3' => InkColor::Orange,
...
fn next_ink_color(color: kf_draw_core::InkColor) -> kf_draw_core::InkColor {
    use kf_draw_core::InkColor;
    match color {
...
fn recolor_selection(app: &mut App, color: kf_draw_core::InkColor) {
...
fn color_name(color: kf_draw_core::InkColor) -> &'static str {
    use kf_draw_core::InkColor;
    match color {
...
fn next_line_style(s: kf_draw_core::LineStyle) -> kf_draw_core::LineStyle {
    use kf_draw_core::LineStyle;
    match s {
...
fn line_style_name(s: kf_draw_core::LineStyle) -> &'static str {
    use kf_draw_core::LineStyle;
    match s {
...
fn next_box_style(s: kf_draw_core::BoxStyle) -> kf_draw_core::BoxStyle {
    use kf_draw_core::BoxStyle;
    match s {
...
fn box_style_name(s: kf_draw_core::BoxStyle) -> &'static str {
    use kf_draw_core::BoxStyle;
    match s {
...
fn next_text_border(s: kf_draw_core::TextBorderMode) -> kf_draw_core::TextBorderMode {
    use kf_draw_core::TextBorderMode;
    match s {
...
fn text_border_name(s: kf_draw_core::TextBorderMode) -> &'static str {
    use kf_draw_core::TextBorderMode;
    match s {
...
fn align_selection(app: &mut App, how: kf_draw_core::Align) {
...
fn align_name(how: kf_draw_core::Align) -> &'static str {
    match how {
        kf_draw_core::Align::Left => "left edge",
        kf_draw_core::Align::Right => "right edge",
        kf_draw_core::Align::Top => "top edge",
        kf_draw_core::Align::Bottom => "bottom edge",
        kf_draw_core::Align::HorizontalCenter => "horizontal center",
        kf_draw_core::Align::VerticalCenter => "vertical center",
    }
}

fn distribute_selection(app: &mut App, axis: kf_draw_core::DistributeAxis) {
...
fn distribute_name(axis: kf_draw_core::DistributeAxis) -> &'static str {
    match axis {
        kf_draw_core::DistributeAxis::Horizontal => "equal horizontal spacing",
        kf_draw_core::DistributeAxis::Vertical => "equal vertical spacing",
    }
}
