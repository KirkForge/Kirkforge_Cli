...
fn run() -> anyhow::Result<std::process::ExitCode> {
    ...
    let state = match cli.load.as_deref() {
        Some(path) => {
            let doc = load_doc(path)?;
            kf_draw_core::DrawState::with_document(doc)
        }
        None => kf_draw_core::DrawState::new(),
    };
...
fn validate_exit_code(report: &kf_draw_core::ValidateReport) -> std::process::ExitCode {
    if report.is_ok() {
