...
use kf_draw_core::{build_scene, load_document, render_plain, validate_document};
...
pub fn load_doc(path: &str) -> Result<kf_draw_core::DrawDocument> {
...
pub fn run_validate(path: &str) -> Result<kf_draw_core::ValidateReport> {
    crate::event::validate_path_arg(path)?;
    let json = fs::read_to_string(path).with_context(|| format!("read {path}"))?;
...
/// it.
pub fn format_validate_report(report: &kf_draw_core::ValidateReport, path: &str) -> String {
    use kf_draw_core::ObjectKind;
...
pub fn format_validate_report_json(
    report: &kf_draw_core::ValidateReport,
    path: &str,
...
fn kind_label(k: kf_draw_core::ObjectKind) -> &'static str {
    match k {
        kf_draw_core::ObjectKind::Box => "box",
        kf_draw_core::ObjectKind::Line => "line",
        kf_draw_core::ObjectKind::Elbow => "elbow",
        kf_draw_core::ObjectKind::Paint => "paint",
        kf_draw_core::ObjectKind::Text => "text",
    }
}

/// Render a document as a fenced markdown code block.
pub fn render_fenced(doc: &kf_draw_core::DrawDocument) -> String {
...
pub fn render_ansi(doc: &kf_draw_core::DrawDocument) -> String {
...
fn emit_ansi_row(out: &mut String, row: &[kf_draw_core::SceneCell]) {
...
fn ansi_color_code(c: Option<kf_draw_core::InkColor>) -> Option<i32> {
