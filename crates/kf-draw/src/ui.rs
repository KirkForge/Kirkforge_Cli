...
use kf_draw_core::{compose_scene, create_scene, object::get_object_bounds, Rect};
...
pub fn draw(app: &mut App, frame: &mut Frame) {
    ...
    if let Some(scene) = scene {
        ...
        if let (Some(origin), Some(bounds)) = (app.scene_origin, app.state.selection_bounds()) {
            ...
            if sel.len() == 1 && matches!(sel[0], kf_draw_core::DrawObject::Box(_)) {
        ...
        if let (Some(origin), Some(edit)) = (app.scene_origin, app.text_edit.as_ref()) {
            if let Some(target) = app.state.text_object(&edit.target_id) {
                let cursor = kf_draw_core::text_edit_cursor_position(
                    target,
                    &edit.buffer,
                    edit.cursor_offset,
                );
    ...
    let status_text = if app.find_active() {
        ...
        let matches = kf_draw_core::filter_palette(app.palette_buffer());
    ...
    } else if let Some(summary) = kf_draw_core::selection_summary(&app.state) {
        ...
        format!(
            ...
            kf_draw_core::format_summary(&summary),
        )
    } else {
        format!(
            " {} | tool={:?} | scroll=({}, {}) | objects={}",
...
fn render_layers_panel(
    ...
    let layers = kf_draw_core::layer_list(state);
    ...
    for (idx, layer) in layers.iter().enumerate() {
        ...
        let kind = kf_draw_core::kind_label(layer.kind);
...
fn render_inspector_panel(
    ...
    if count == 0 {
    ...
    } else if let Some(summary) = kf_draw_core::selection_summary(state) {
        ...
        for row in kf_draw_core::format_summary_rows(&summary) {
