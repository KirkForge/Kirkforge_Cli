...
mod tests {
    ...
    fn fast_lib_is_fast() {
        let p = Profile {
            binaries: vec![bin("kf-code", "lib", 4_000, 1285, 2)],
    ...
    fn long_binary_with_many_tests_is_medium() {
        ...
        let p = Profile {
            binaries: vec![bin("kf-code", "lib", 30_000, 100, 0)],
    ...
    fn per_test_fast_test_is_fast() {
        let p = per_test_profile(vec![test_profile("a", "kf-code", 50, true, false)], false);
    ...
    fn per_test_slow_single_test_is_slow_even_in_a_fast_binary() {
        ...
        let p = per_test_profile(
            vec![test_profile("slow_one", "kf-code", 3000, true, false)],
            false,
        );
        let c = classify_per_test(&p);
    ...
    fn per_test_carries_coarse_flag() {
        let p = per_test_profile(vec![test_profile("a", "kf-code", 100, true, false)], true);
        let c = classify_per_test(&p);
        assert!(c.coarse);
    }
    ...
    fn per_test_failing_test_is_medium_not_slow() {
        let p = per_test_profile(
            vec![test_profile("fail", "kf-code", 100, false, false)],
            false,
        );
        let c = classify_per_test(&p);
        assert_eq!(c.tests[0].speed, Speed::Medium);
