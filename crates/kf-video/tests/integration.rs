...
use kf_video::compose::Composition;
use kf_video::orchestrator::{Checkpoint, Stage};
use kf_video::pipelines::{AnimatedExplainer, Pipeline};
use kf_video::tools::ToolRegistry;
...
async fn animated_explainer_renders_end_to_end() {
    ...
    kf_video::orchestrator::run_pipeline(&pipe, &dir_path, &reg)
        .await
        .expect("pipeline");
    ...
    match &comp.audio {
        Some(kf_video::compose::AudioSpec::Narration { path, .. }) => {
            assert!(
                path.exists(),
...
async fn brief_parser_round_trip() {
    ...
    let b = kf_video::pipelines::brief::parse_brief(
        "Title\nSubtitle\n- 50% claim\n- $4.2B market\n- 3.2x lift\n> brand.tag\n",
    );
    ...
    let plan = kf_video::pipelines::brief::scene_plan_from_brief(&b);
...
async fn assets_stage_probes_clip_cut_via_analyzer() {
    ...
    use kf_video::orchestrator::checkpoint::{Checkpoint, StageRecord};
...
async fn compose_refuses_unimplemented_render_runtime() {
    ...
    let pipe = AnimatedExplainer;
    let reg = ToolRegistry::with_builtins();
    let err = kf_video::orchestrator::run_pipeline(&pipe, &dir_path, &reg)
        .await
...
async fn caption_overlay_writes_srt_sidecar_and_embeds_subtitle_stream() {
    ...
    use kf_video::compose::{render_composition, AudioSpec, Composition, Scene};
    let dir = tempfile::tempdir().expect("tempdir");
...
async fn media_profile_unknown_name_errors() {
    ...
    use kf_video::compose::{get_profile, ALL_PROFILES};
