...
fn ts_interface_merge_dedupes_within_file() {
    ...
    let before = idx
        .symbols()
        .iter()
        .filter(|s| s.kind == kirkforge_context_index::SymbolKind::Interface)
        .count();
    assert_eq!(
        before, 3,
...
fn ts_interface_merge_dedup_keeps_one_per_file() {
    ...
    let mut idx = index_fixture("ts_interface_merge.ts");
    idx.dedup_interfaces();
    let foo_entries: Vec<&kirkforge_context_index::Symbol> = idx
        .symbols()
        .iter()
        .filter(|s| s.name == "Foo" && s.kind == kirkforge_context_index::SymbolKind::Interface)
        .collect();
    assert_eq!(
        foo_entries.len(),
        1,
...
fn py_decorator_function_extracted_by_name() {
    ...
    let syms: Vec<&kirkforge_context_index::Symbol> = idx
        .symbols()
        .iter()
        .filter(|s| s.kind == kirkforge_context_index::SymbolKind::Function)
        .collect();
    let names: Vec<&str> = syms.iter().map(|s| s.name.as_str()).collect();
