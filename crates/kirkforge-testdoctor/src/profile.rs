...
";
        ...
        assert_eq!(tests[0].binary, "kirkforge");
...
";
    ...
    fn per_test_from_binary_profiles_splits_pass_fail() {
        ...
        // 4 passed + 1 failed from kirkforge; ignored-only binary yields none.
        assert_eq!(tests.len(), 5);
        assert_eq!(tests[0].duration_ms, 200);
        assert!(tests[0].passed);
        assert!(!tests[4].passed);
        assert_eq!(tests[4].name, "kirkforge::failed_0");
    }
}
