...
The workspace has one binary crate (`kf-code`) and 12 active satellite crates
under `crates/`. Four additional crates are excluded from the default build
(`kf-video`, `kf-compress-hosts`, `kf-compress-cli`, `kf-budget-cli`) — they
exist on disk and can be built explicitly with `-p`.

```
kf-code (root bin)          ← the CLI the user runs
├── src/                       ← agent core (session, tools, TUI, adapters, verifiers)
├── crates/                    ← 12 active + 4 excluded satellite crates
...
│   ├── kf-draw         ← kfd: terminal diagram editor binary (default-OFF)
...
│   └── kf-testdoctor   ← test-performance doctor
│   (excluded: kf-video, kf-compress-hosts, kf-compress-cli, kf-budget-cli)
...
Three more crates link in when their feature flag is enabled
(`stratum` → `kf-compress-core`, `budget` → `kf-budget-core`,
`draw` → `kf-draw-core`). The remaining crates are excluded from
the workspace or are satellite binaries.
...
by content type and applies minification plus head/tail truncation with
bloat offload *before* they enter the context window. Four modes: `off`,
`lite`, `full`, `ultra`.
