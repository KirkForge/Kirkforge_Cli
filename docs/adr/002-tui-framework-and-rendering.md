...
The Rust CLI needs a terminal user interface. It is used on the developer's daily driver machine, which may be a laptop or workstation with a conventional terminal emulator. The UI requirements are:
...
| termion | Linux-only; also stale, unmaintained |
...
loop {
    ...
    // Yield to OS when idle — keeps CPU use low on battery-powered machines
...
**Negative:**
- Immediate-mode means full redraw each frame. On low-resolution or slow terminals this might flicker. Mitigation: double-buffer (ratatui does this by default via crossterm's alternate screen), and dirty-flag checks to skip redraw when nothing changed
...
- Should syntax highlighting be lazy (render-on-scroll) or eager (highlight the visible window)? Lazy is cheaper when the host is busy. Answer this during milestone 2 when tool views land.
