...
Fold `kf-video` into the main binary behind a **non-default** `video` feature flag. When enabled, the 8 video tools are direct Rust calls. When disabled (the default), video is not compiled and not registered — the binary is smaller.
...
- Feature flag: `video` (non-default, optional dep on `kf-video`).
...
- `kf-video` binary remains standalone.
