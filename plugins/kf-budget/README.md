# Kf-Code-Plugin3 plugin for Kf-Code-Cli

This directory is a Kf-Code filesystem plugin that exposes the `plugin3` output-side token-budget tool as tools, a skill, and lifecycle hooks inside the `kf-code` TUI/CLI. The `plugin3` binary builds from `crates/plugin3-cli` in this workspace.
...
2. Copy this directory into the Kf-Code plugins folder:
   ...
   cp -R plugins/kf-code-plugin3 ~/.local/share/kf-code/plugins/kf-code-plugin3
   ```

3. Set `max_plugin_trust = "shell"` (or higher) in `~/.local/share/kf-code/config.toml`, because the plugin shells out to `plugin3`.

4. Restart `kf-code run`. The TUI status bar should show the plugin3 tools loaded.
...
| Kf-Code event | plugin3 hook | Script |
|-----------------|--------------|--------|
...
MIT — same as the rest of Kf-Code-Plugin3.
