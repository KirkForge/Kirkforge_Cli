...
impl ModelAdapter for AnthropicAdapter {
    ...
    async fn stream(
        ...
        let body = build_anthropic_body(&self.model, messages, tools, self.json_mode, self.seed, self.extended_thinking, self.budget_tokens);
...
mod tests {
    ...
    fn body_hoists_system_messages() {
        ...
        let body = build_anthropic_body("claude-sonnet-4", &messages, &[], false, None, false, 10_000);
        assert!(body
            .get("messages")
            .unwrap()
            .as_array()
    ...
    fn body_user_with_single_image_part_emits_image_block() {
        ...
        let body = build_anthropic_body("claude-3-opus", &messages, &[], false, None, false, 10_000);
    ...
    fn body_tools_block_has_cache_control_on_last_tool() {
        ...
        let body = build_anthropic_body("claude-sonnet-4", &messages, &tools, false, None, false, 10_000);
        let tools_arr = body["tools"].as_array().unwrap();
