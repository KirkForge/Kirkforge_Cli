...
pub mod go_vet;
mod helpers;
/// Verifier bus — deterministic post-execution checks and correction loop.
///
/// Verifiers receive a `VerifyContext` (changed files, tool name, bash
/// command) from the dispatch layer and react to specific tool outcomes.
...
/// # Correction loop
///
/// When a verifier finds an issue with a `FixSuggestion`, the correction
/// loop processes it. The correction loop decides whether to auto-fix
/// (deterministic) or report back to the model.
pub mod lint;
pub mod node_lint;
pub mod node_test;
pub mod plugin;
