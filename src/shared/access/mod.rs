...
/// In debug builds, `warn_if_unsandboxed` is used instead. The `--i-accept-unsandboxed`
/// flag exists only in debug builds; release builds refuse unconditionally.
pub fn refuse_if_production_unsandboxed(path_guard: &PathGuard) -> anyhow::Result<()> {
    if path_guard.is_sandboxed() {
        return Ok(());
    }
    anyhow::bail!(
        "Refused to start: no `sandbox_dir` configured and this is a release build. \
         Model-driven writes are unrestricted. Set `sandbox_dir = \".\"` in config.toml \
         (or KF_CODE_SANDBOX_DIR) for a minimal safe default, or list `allowed_write_dirs` \
         to scope writes."
    );
}
