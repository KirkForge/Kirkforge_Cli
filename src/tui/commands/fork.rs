...
pub async fn handle_resume_command(
    ...
    match ConversationLog::open(fork.path.clone()) {
        Ok(fork_log) => {
            ...
            // Reload the TUI's display list from the fork's persisted
            // history. `messages_to_entries` maps the executor's
            // `Message` enum into the TUI's `ConversationEntry` struct
            // (the two have different shape — `Message` is the
            // wire/storage form with a `Role` enum, `ConversationEntry`
            // is the TUI display form with a `role: String` field).
