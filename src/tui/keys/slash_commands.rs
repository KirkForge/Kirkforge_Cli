...
pub(crate) const COMMANDS: &[SlashCommand] = &[
    ...
    SlashCommand {
        ...
        usage: "/thinking shows or hides thinking content.",
        group: "Advanced",
    },
    SlashCommand {
        triggers: &["/theme"],
...
pub(crate) fn help_text(skill_registry: &SkillRegistry) -> String {
    ...
    out.push_str(
        "\nBash passthrough:\n\
         ...
         \n  Esc      Cancel / close: dismisses slash menu, file completer, or active overlay tab (cancel-only — never toggles)\n\
         \nStatus bar:\n\
        ...
        "/thinking" => {
            ...
            state
                ...
                .push_back(ConversationEntry::new(
                    "system",
                    format!("Thinking blocks are now {status}. Run /thinking again to toggle."),
                ));
            Ok(true)
        }
        "/plan" => {
