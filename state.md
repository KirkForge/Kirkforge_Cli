...
## Session 2026-08-13 — WO 30.6 + WO 30.1 (branch `wo30a`, worktree `.worktrees/wo30a`, 1 commit, NOT pushed)

Two subagent isolation items, both centered on `src/session/task_spawner.rs`:
- **WO 30.6 — subagent approval forwarding (full fix):** `InProcessTaskSpawner` gains a `parent_approval` forwarder (`Arc<Mutex<Option<Sender>>>`) set from `Executor::run_turn` (the session-stable approval channel, set idempotently each turn — the common chokepoint for main-loop + subagent + test paths). `run_task`'s approval handler now forwards subagent destructive-tool requests to the parent's channel (the request carries its own responder, so the parent's existing TUI/line-mode handler decides and the response routes back with no extra plumbing). With no parent channel (top-level scheduled job) the P0 policy from `5fbd955` still applies (auto-approve in CI, deny otherwise). Nesting: each `run_task` pins its subagent executor's spawner to its own approval channel → N-deep chains forward level-by-level to the top-level user. Layering: the `TaskSpawner` trait was left unchanged so the tools layer doesn't reach into `session::executor` (the WO 28.1 invariant); the forwarder lives on the session-layer struct.
- **WO 30.6 prerequisite fix (disclosed):** the executor built `self.task_spawner` (`build_task_spawner`) but `run_prepared_call` hardcoded `task_spawner: None` in the dispatch `ToolContext` and `PreparedCall` had no spawner field — so the parent `task` tool always errored "task tool is not available in this context" (latent gap since WO 28.1; no test exercised the dispatch→task path, only `ToolContext::with_spawner` bypasses). Fixed: spawner threaded through `PreparedCall` → `ctx.task_spawner`; executor field held concretely (`Arc<InProcessTaskSpawner>`) so the approval forwarder can be set on it.
- **WO 30.1 — `coder` subagent worktree isolation:** for the `coder` persona, `run_task` creates a git worktree via the existing `WorktreeSession::create` and points the path guard's `sandbox_dir` at it, confining `read_file`/`write_file`/`edit_file`/`notebook_edit` to an isolated checkout. The worktree path is returned in the summary for the parent to review/merge. `explore`/`plan` keep reading the parent workspace. Best-effort: non-git dir → fall back to shared workspace + warn.

### Pending / deferred (this session)
- **WO 30.1 — bash CWD confinement (DEFERRED):** the worktree confines the file tools via `path_guard.sandbox_dir`, but bash runs in the process CWD (the parent's workspace), NOT the worktree. Bash keeps its existing landlock/sandbox posture, so this is an additional-isurface gap rather than a regression. Remaining work: point the bash tool's workdir resolution at the worktree (the bash `workdir` arg defaults to `"."` relative to the process CWD) — needs a per-subagent CWD override in the bash tool / runner. Tracked in WO 30.1-later.
- **WO 30.1 — completed-worktree cleanup (DEFERRED by design):** completed `coder` worktrees persist on disk for the parent to review/merge (cleaned up on error via `Drop`). The user must remove them manually after merging. Remaining work: a janitor / `git worktree prune` sweep, or an explicit `task_merge` tool. Tracked in WO 30.1-later.
